...
def main():
    ...
    ssh_client.close()
    print('[System] Successfully synchronized databases')
...
def get_host_configuration():
    if os.path.isfile('host.json'):
        with open('host.json', 'r') as read_file:
            ...
            print('[Local] Load host configuration')
    else:
        sys.exit('[Error] Local host configuration not found')
...
def load_pip_modules():
    ...
    try:
        ...
        sys.exit('[Error] Pip is not installed')

    print('[Local] Load pip modules')
    package = 'paramiko'
    try:
...
def get_remote_password():
    ...
    _password = getpass.getpass('[System] SSH password for user "' + config['host']['remote']['user'] + '": ')
    remote_ssh_password = _password

def check_local_configuration():
    if os.path.isfile(config['host']['local']['path']) == False:
        sys.exit('[Error] Local database configuration not found')
    ...
    print('[Local] Checking database configuration')
...
def check_remote_configuration():
    ...
    print('[Remote] Checking database configuration')
...
def create_remote_database_dump():
    ...
    print('[Remote] Creating database dump')
...
def prepare_remote_database_dump():
    print('[Remote] Prepare database dump')
...
def get_remote_database_dump():
    ...
    print('[Remote] Download database dump')
    #
...
def import_database_dump():
    ...
    print('[Local] Import database dump')
...
def prepare_local_database_dump():
    print('[Local] Prepare database dump')
...
def check_local_database_dump():
    with open(os.path.abspath(os.getcwd()) + '/.sync/' + remote_database_dump_file_name) as f:
        ...
        if "-- Dump completed on" not in lines[-1]:
            sys.exit('[Error] Dump was not fully transferred')
...
def get_ssh_client():
    ...
    print('[Remote] Successfully connect to SSH client')
    return ssh_client
...
def download_status(sent,size):
    ...
    sys.stdout.write("[Remote] Download status: {0} MB of {1} MB sent".
        format(sent_mb, size,))
    sys.stdout.write('\r')
