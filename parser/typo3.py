...
def check_target_configuration():
    ...
    _target_db_config = check_output(
        [helper.get_command('target','php'), '-r', 'echo json_encode(include "' + system.config['host']['target']['path'] + '");'])
    ...
    if system.option['verbose']:
            output.message(output.get_subject().TARGET, output.get_bcolors().BLACK + helper.get_command('target','php') + ' -r "echo json_encode(include "' + system.config['host']['target']['path'] + '");"' + output.get_bcolors().ENDC, True)
